...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    static get observedAttributes() {
      return ['shape', 'radius', 'mask', 'fit', 'placeholder', 'src', 'id', 'credit', 'credit-href', 'lock'];
    }

    _locked() { return this.hasAttribute('lock') && this.getAttribute('lock') !== 'false'; }
    ...
    constructor() {
      ...
      root.addEventListener('click', (e) => {
        ...
        if (!this.hasAttribute('data-editable') || this._locked()) return;
        if (act === 'replace') {
      ...
      this.addEventListener('dblclick', (e) => {
        if (!this.hasAttribute('data-editable') || this._locked() || !this._reframes()) return;
        e.preventDefault();
        if (this.hasAttribute('data-reframe')) this._exitReframe(true);
        else this._enterReframe();
      });
    ...
    _render() {
      ...
      const editable = !!(window.omelette && window.omelette.writeFile) && !this._locked();
